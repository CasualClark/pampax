...
export interface CostAnalysis {
  ...
  costEfficiency: number; // satisfaction per cost unit
  costTrend: TimeSeriesData;
}

/**
 * Intent-specific performance metrics
 */
...
/**
 * Comparison report between two time periods or configurations
 */
...
export interface TrendAnalysis {
  trends: Record<string, {
    ...
    forecast: TimeSeriesData; // predicted future values
  }>;
  seasonalPatterns: Record<string, TimeSeriesData>;
  anomalies: Array<{
    timestamp: Date;
    metric: string;
    value: number;
...
export interface PerformanceTrackerConfig {
  tokenCostPerMillion?: number; // Default: $0.002
  significanceThreshold?: number; // Default: 0.05 for 5% significance
  forecastHorizon?: number; // Default: 30 days
  minSampleSize?: number; // Default: 100 interactions
...
export class PerformanceTracker {
   ...
   */
  async trackMetrics(signals: OutcomeSignal[], period: TimePeriod): Promise<PerformanceMetrics> {
  ...
  async generateComparisonReport(
    ...
    try {
      ...
      for (const [language, afterLang] of Object.entries(after.languagePerformance)) {
        const beforeLang = before.languagePerformance[language];
        if (!beforeLang) continue;

        const winRateChange = afterLang.winRate - beforeLang.winRate;
        const metricKey = `language_${language}_winRate`;
  ...
  async analyzeTrends(
    ...
    try {
      ...
      // Analyze intent-specific trends
      const allIntents = new Set<string>();
      metrics.forEach(m => Object.keys(m.intentPerformance).forEach(intent => allIntents.add(intent)));

      for (const intent of allIntents) {
        const intentData = metrics
          .map(m => {
            const intentMetrics = m.intentPerformance[intent];
            return intentMetrics ? {
              timestamp: m.period.start,
              value: intentMetrics.winRate
            } : null;
          })
          .filter(Boolean) as TimeSeriesPoint[];

        if (intentData.length >= 3) {
          trends[`intent_${intent}`] = this.analyzeTimeSeries(intentData, `intent_${intent}`);
        }
      }
   ...
   * Filter signals by time period
   */
  private filterSignalsByPeriod(signals: OutcomeSignal[], _period: TimePeriod): OutcomeSignal[] {
   ...
   */
  private async computeWinRates(signals: OutcomeSignal[]): Promise<Record<string, number>> {
   ...
   */
  private async computeSatisfactionTrends(
    ...
    const overallPoints: TimeSeriesPoint[] = [];
   ...
   */
  private async computeTokenCostAnalysis(signals: OutcomeSignal[]): Promise<CostAnalysis> {
    ...
    const costTrend: TimeSeriesData = {
      label: 'Token Cost Trend',
      points: [{
        timestamp: new Date(),
        value: totalCost
      }],
   ...
   */
  private async computeIntentPerformance(
    signals: OutcomeSignal[],
    _period: TimePeriod
    ...
    for (const [intent, group] of Object.entries(intentGroups)) {
      ...
      const trend: TimeSeriesData = {
        label: `${intent} Performance Trend`,
        points: [{
          timestamp: new Date(),
          value: winRate
        }],
   ...
   */
  private async computeLanguagePerformance(
    signals: OutcomeSignal[],
    _period: TimePeriod
  ): Promise<LanguageMetrics> {
    // Group by inferred language (placeholder implementation)
    const languageGroups: Record<string, OutcomeSignal[]> = {
    ...
    for (const [language, group] of Object.entries(languageGroups)) {
      ...
      const trend: TimeSeriesData = {
        label: `${language} Performance Trend`,
        points: [{
          timestamp: new Date(),
          value: winRate
        }],
   ...
   */
  private async computeRepositoryPerformance(
    signals: OutcomeSignal[],
    _period: TimePeriod
  ): Promise<RepoMetrics> {
   ...
   */
  private calculateOverallSatisfactionRate(signals: OutcomeSignal[]): number {
   ...
   */
  private calculateOverallCostEfficiency(signals: OutcomeSignal[], costAnalysis: CostAnalysis): number {
   ...
   */
  private analyzeTimeSeries(points: TimeSeriesPoint[], metricName: string): any {
    if (points.length < 2) {
      return {
        direction: 'stable',
        rate: 0,
   ...
   */
  private detectAnomalies(metrics: PerformanceMetrics[]): Array<any> {
   ...
   */
  private generateTrendInsights(trends: Record<string, any>, anomalies: Array<any>): string[] {
    const insights: string[] = [];
   ...
   */
  private exportToMarkdown(metrics: PerformanceMetrics): string {
    const lines: string[] = [];
