...
/**
 * Search context for policy evaluation
 */
...
/**
 * Policy decision structure that controls search behavior
 */
...
/**
 * Policy configuration for different intent types
 */
...
export interface RepositoryPolicyConfig {
  [repoName: string]: Partial<PolicyConfig>;
}
...
export class PolicyGate {
  private repositoryPolicies: RepositoryPolicyConfig = {};
  private defaultPolicies: PolicyConfig;
   ...
   */
  private getBasePolicy(intentType: string): PolicyDecision {
    const policy = this.defaultPolicies[intentType as keyof PolicyConfig];
    if (!policy) {
   ...
   */
  private getRepositoryPolicy(repoName?: string): Partial<PolicyConfig> | undefined {
    if (!repoName) {
      return undefined;
    }
  ...
  private mergePolicies(
    base: PolicyDecision, 
    override?: Partial<PolicyDecision>
  ): PolicyDecision {
    if (!override) {
      return base;
    }
  ...
  private adjustWeightsForLanguage(
    ...
    switch (language.toLowerCase()) {
      case 'python':
        adjusted['definition'] *= 1.2; // Python values definitions highly
        adjusted['implementation'] *= 1.1;
        break;
      case 'typescript':
      case 'javascript':
   ...
   */
  private loadPoliciesFromConfig(): PolicyConfig | null {
    try {
  ...
  getPolicy(repoName: string, intentType: string): PolicyDecision {
    ...
    if (repositoryPolicy && repositoryPolicy[intentType as keyof PolicyConfig]) {
      return this.mergePolicies(basePolicy, repositoryPolicy[intentType as keyof PolicyConfig]);
    }
