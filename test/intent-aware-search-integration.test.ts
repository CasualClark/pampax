...
// Mock the CLI command for testing
...
describe('Intent-Aware Search Integration Tests', () => {
  ...
  let seedMixOptimizer: SeedMixOptimizer;
  ...
  test('should perform end-to-end intent-aware search workflow', async () => {
    ...
    assert.ok(intent.confidence > 0.5, 'Should have good confidence');
    ...
    assert.equal(policy.maxDepth, 2, 'Should have appropriate max depth');
    ...
    assert.ok(seedConfig.confidenceMultiplier > 1.0, 'Should apply confidence multiplier');
  });
  ...
  test('should handle config intent search workflow', async () => {
    ...
    assert.equal(policy.maxDepth, 1, 'Should have limited depth for config intent');
  ...
  test('should handle API intent search workflow', async () => {
    ...
    assert.equal(policy.earlyStopThreshold, 2, 'Should have low early stop threshold for API');
  ...
  test('should handle incident intent search workflow', async () => {
    ...
    assert.equal(policy.maxDepth, 3, 'Should have high max depth for incident');
    assert.equal(policy.earlyStopThreshold, 5, 'Should have higher early stop threshold for incident');
  ...
  test('should handle general search intent workflow', async () => {
    ...
    assert.equal(policy.earlyStopThreshold, 10, 'Should have generous early stop threshold');
    
    // Optimize seed mix
    const seedConfig = seedMixOptimizer.optimize(intent, policy);
    assert.equal(seedConfig.vectorWeight, 1.0, 'Should have balanced vector weight');
    assert.equal(seedConfig.bm25Weight, 1.0, 'Should have balanced BM25 weight');
    assert.equal(seedConfig.memoryWeight, 1.0, 'Should have balanced memory weight');
    assert.equal(seedConfig.symbolWeight, 1.0, 'Should have balanced symbol weight');
  });
  ...
  test('should handle low confidence scenarios', async () => {
    ...
    assert.ok(policy.earlyStopThreshold <= 9, 'Should reduce early stop threshold with low confidence');
    
    // Optimize seed mix
    const seedConfig = seedMixOptimizer.optimize(intent, policy);
    assert.ok(seedConfig.confidenceMultiplier < 1.0, 'Should reduce weights with low confidence');
  });
  ...
  test('should handle repository-specific overrides', async () => {
    ...
    const query = 'React component render method';
    const intent = intentClassifier.classify(query);
    const searchContext = {
      repo: 'frontend-repo',
      language: 'typescript'
    ...
    assert.equal(policy.maxDepth, 4, 'Should use repository-specific max depth');
    assert.equal(policy.earlyStopThreshold, 6, 'Should use repository-specific early stop threshold');
    
    const seedConfig = seedMixOptimizer.optimize(intent, policy);
    assert.equal(seedConfig.maxDepth, 4, 'Seed config should respect repository override');
  });
  ...
  test('should handle language-specific adjustments', async () => {
    ...
    const javaSeedConfig = seedMixOptimizer.optimize(intent, javaPolicy);
  ...
  test('should handle forced intent overrides', async () => {
    ...
    assert.equal(policy.maxDepth, 2, 'Should use symbol policy for forced intent');
  ...
  test('should maintain backward compatibility', async () => {
    ...
    assert.equal(defaultIntent.confidence, 0, 'Should have zero confidence for empty query');
  });
