...
describe('Intent System Performance Tests', () => {
  ...
  test('should measure early stop performance', () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      // Verify correctness
      assert.ok(earlyStoppedResults.length <= config.earlyStopThreshold, 'Should apply early stop');
    }
  ...
  test('should measure memory usage and caching efficiency', () => {
    ...
    assert.ok(cacheSpeedup > 2.0, `Cache should provide at least 2x speedup, got ${cacheSpeedup.toFixed(2)}x`);
    assert.ok(metrics.cacheHitRate > 0.8, `Cache hit rate should be > 80%, got ${(metrics.cacheHitRate * 100).toFixed(1)}%`);
  });

  test('should measure performance under load', () => {
    ...
    const promises = Array.from({ length: concurrentRequests }, async (_, workerId) => {
      ...
      for (let i = 0; i < requestsPerWorker; i++) {
        ...
        // Full intent-aware search workflow
        const intent = intentClassifier.classify(query);
        const policy = policyGate.evaluate(intent, {});
        const seedConfig = seedMixOptimizer.optimize(intent, policy);
        
        const endTime = performance.now();
        workerTimes.push(endTime - startTime);
      }
    ...
    Promise.all(promises).then(workerResults => {
      workerResults.forEach(workerTimes => {
        loadTestTimes.push(...workerTimes);
      });

      const avgTime = loadTestTimes.reduce((sum, time) => sum + time, 0) / loadTestTimes.length;
      const maxTime = Math.max(...loadTestTimes);
      const p95Time = loadTestTimes.sort((a, b) => a - b)[Math.floor(loadTestTimes.length * 0.95)];
      const totalRequests = concurrentRequests * requestsPerWorker;

      console.log(`Load Test Performance (${totalRequests} total requests):`);
      console.log(`  Average: ${avgTime.toFixed(3)}ms`);
      console.log(`  Max: ${maxTime.toFixed(3)}ms`);
      console.log(`  95th percentile: ${p95Time.toFixed(3)}ms`);
      console.log(`  Requests per second: ${(1000 / avgTime).toFixed(1)}`);

      assert.ok(avgTime < 20.0, `Load test average should be < 20ms, got ${avgTime.toFixed(3)}ms`);
      assert.ok(maxTime < 100.0, `Load test max should be < 100ms, got ${maxTime.toFixed(3)}ms`);
      assert.ok(p95Time < 40.0, `Load test 95th percentile should be < 40ms, got ${p95Time.toFixed(3)}ms`);
    });
  });
  ...
  test('should validate performance regression thresholds', () => {
    ...
    const iterations = 10;
    const times: { intent: number[]; policy: number[]; seed: number[]; total: number[] } = {
      intent: [],
      policy: [],
      seed: [],
      total: []
    ...
    const calculateStats = (timeArray: number[]) => {
