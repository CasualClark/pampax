...
import { SignatureCache, type CacheEntry, type CacheStats } from '../../src/learning/signature-cache.js';
...
describe('SignatureCache', () => {
  test('should store and retrieve cache entries', async () => {
    ...
    try {
      ...
      assert.strictEqual(retrieved.usageCount, 1);
    } finally {
      await cache.cleanup();
    }
  });
  ...
  test('should not cache entries below satisfaction threshold', async () => {
    ...
    try {
      const entry: CacheEntry = {
        querySignature: 'low_satisfaction_query',
        bundleId: 'bundle_789',
        satisfaction: 0.7, // Below 0.8 threshold
        usageCount: 1,
        createdAt: Date.now(),
        lastUsed: Date.now(),
        ttl: 7 * 24 * 60 * 60 * 1000
  ...
  test('should update usage count and last used timestamp on retrieval', async () => {
    ...
    try {
      const entry: CacheEntry = {
        querySignature: 'usage_test_query',
        bundleId: 'bundle_usage',
        satisfaction: 0.85,
        usageCount: 1,
  ...
  test('should provide accurate cache statistics', async () => {
    ...
    try {
      ...
      // Add an entry
      const entry: CacheEntry = {
        querySignature: 'stats_test_query',
        bundleId: 'bundle_stats',
        satisfaction: 0.9,
        usageCount: 1,
        createdAt: Date.now(),
        lastUsed: Date.now(),
  ...
  test('should invalidate all cache entries when no pattern provided', async () => {
    ...
    try {
      // Add entries
      const entry: CacheEntry = {
        querySignature: 'clear_test_query',
  ...
  test('should clean up expired entries', async () => {
    ...
    try {
      const expiredEntry: CacheEntry = {
        querySignature: 'cleanup_expired',
        bundleId: 'bundle_cleanup_expired',
        satisfaction: 0.9,
        usageCount: 1,
        createdAt: Date.now() - 2000,
        lastUsed: Date.now() - 2000,
      ...
      const validEntry: CacheEntry = {
        querySignature: 'cleanup_valid',
        bundleId: 'bundle_cleanup_valid',
        satisfaction: 0.9,
        usageCount: 1,
        createdAt: Date.now(),
        lastUsed: Date.now(),
  ...
  test('should handle concurrent access safely', async () => {
    ...
    try {
      const entry: CacheEntry = {
        querySignature: 'concurrent_test',
        bundleId: 'bundle_concurrent',
        satisfaction: 0.9,
        usageCount: 1,
        createdAt: Date.now(),
        lastUsed: Date.now(),
